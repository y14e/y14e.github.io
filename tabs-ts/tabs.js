...
export default class Tabs {
    ...
    constructor(root, options = {}) {
        ...
        this.#listElements = [
            ...this.#rootElement.querySelectorAll(`${this.#settings.selector.list}${NOT_NESTED}`),
        ];
        ...
        this.#tabElements = [
            ...this.#rootElement.querySelectorAll(`${this.#settings.selector.tab}${NOT_NESTED}`),
        ];
        ...
        this.#indicatorElements = [
            ...this.#rootElement.querySelectorAll(`${this.#settings.selector.indicator}${NOT_NESTED}`),
        ];
        ...
        this.#panelElements = [
            ...this.#rootElement.querySelectorAll(`${this.#settings.selector.panel}${NOT_NESTED}`),
        ];
    ...
    async activate(tab, isMatch = false) {
        ...
        this.#tabElements.forEach((tab) => {
            ...
            tab.setAttribute('tabindex', isSelected && !this.#isAvoidedTab(tab) ? '0' : '-1');
        });
        if (!this.#contentElement) {
            return;
        }
        ...
        this.#panelElements.forEach((panel) => {
            ...
            const animation = panel.animate({
                ...
                duration: isMatch || !(fade || crossFade)
                    ? 0
    ...
    async destroy(force = false) {
        ...
        if (!force) {
            await Promise.all(this.#panelElements.map((panel) => this.#bindings.get(panel)?.animation?.finished.catch(() => { })));
        }
        this.#panelElements.forEach((panel) => {
    ...
    #initialize() {
        ...
        this.#tabElements.forEach((tab, i) => {
            ...
            tab.setAttribute('tabindex', tab.ariaSelected === 'true' && !isAvoided ? '0' : '-1');
            if (!isFocusable(tab)) {
                tab.style.setProperty('pointer-events', 'none');
            }
            const labelledBy = new Set(panel.getAttribute('aria-labelledby')?.trim().split(/\s+/) ?? []);
    ...
    #isAvoidedTab(tab) {
        return (this.#settings.avoidDuplicates &&
...
function hasFocusable(container) {
    return !![
        ...container.querySelectorAll(`:is(a[href], area[href], button, embed, iframe, input:not([type="hidden" i]), object, select, details > summary:first-of-type, textarea, [contenteditable]:not([contenteditable="false" i]), [controls], [tabindex]):not(:disabled, [hidden], [inert], [tabindex="-1"])`),
